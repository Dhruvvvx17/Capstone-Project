...
def price_prediction(crop, district):
    crops_list = os.listdir("Karnataka")
    #if not (crop in crops_list):
    try:
        print('5555555555',district,crop)
        return int(default_prices[district][crop])/100,1
    except:
    ...
    # temp_res = df['MeanPrice(Rs./Quintal)'].iloc[-1] 
    # return str(temp_res),0
    ...
    return str(int(round(price[0][0]))/100),0
