...
def find_best_crop(mean_ndvi, standard_dev, crop_category_mean_ndvi):
    ...
    closest = (pd.Series(crop_category_mean_ndvi) -
...
def list_of_crops(lat_in, long_in, district):
    ...
    for url in urls:
        ...
        try:
            ...
            region_of_interst = normalized_ndvi_ndarray[slice_point_top_left_x: slice_point_top_left_x +
            ...
            mean_ndvi /= 40
            print(
                f"mean_ndvi at latitude = {latitude_input} and longitude = {longitude_input} is: {mean_ndvi}")

            '''Save numpy array as PNG'''
            ndvi_image = Image.fromarray(
                np.uint8(cm.gist_earth(region_of_interst)*255))
            ...
            list_of_crops = find_best_crop(
                mean_ndvi, 0.05, crop_category_mean_ndvi)
            # yield pred call
            yields = []
            prices = []
            print(list_of_crops)
